...
func ExportTarget(ctx context.Context, req *rao.ExportTargetReq) ([]map[string]interface{}, error) {
	tx := query.Use(dal.DB()).Target
	targets, err := tx.WithContext(ctx).Where(
		tx.TargetID.In(req.TargetIDs...),
		tx.TeamID.Eq(req.TeamID),
	...
	folderMap := make(map[string]*model.Target, len(targets))
	apiIDs := make([]string, 0, len(targets))
	sqlIDs := make([]string, 0, len(targets))
	tcpIDs := make([]string, 0, len(targets))
	websocketIDs := make([]string, 0, len(targets))
	...
	for _, targetInfo := range targets {
		if targetInfo.TargetType == consts.TargetTypeFolder {
			folderMap[targetInfo.TargetID] = targetInfo
		}
	...
	return packer.ExportTransTargetsToRaoAPIDetails(targets, apiMap, sqlMap, tcpMap, websocketMap, dubboMap, folderMap), nil
}
