...
func Save(ctx *gin.Context, req *rao.MockSaveTargetReq, userID string) (string, error) {
	...
	if req.OperateType == consts.MockOperateTypeSaveAndSync {
		...
		if len(targetInfos) > 0 {
			for _, t := range targetInfos {
				...
				// 保存历史记录
				if _, _, err := lctarget.SaveTargetHistoryRecord(ctx, req.SaveTargetReq); err != nil {
					log.Logger.Error("mock Sync target err:", err)
				}
			}
		}
