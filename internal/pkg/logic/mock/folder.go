...
import (
	...
	lctarget "github.com/Runner-Go-Team/RunnerGo-management-open/internal/pkg/logic/target"
...
func SaveToTarget(ctx *gin.Context, userID string, targetIDs []string, teamID string) error {
	...
	return query.Use(dal.DB()).Transaction(func(tx *query.Query) error {
		...
		for k, mtv := range mockTargets {
			...
			if mtv.TargetType == consts.TargetTypeAPI {
				...
				// 添加 api
				var api *mao.API
				if err := collection.FindOne(ctx, bson.D{{"target_id", mtv.TargetID}}).Decode(&api); err != nil {
					return err
				}
				...
				// 保存历史记录
				req := packer.TransMaoAPIReqToSaveTarget(api)
				req.TargetID = saveTarget.TargetID
				req.ParentID = saveTarget.ParentID
				req.TeamID = saveTarget.TeamID
				req.Name = saveTarget.Name
				req.Method = saveTarget.Method
				req.Sort = saveTarget.Sort
				req.TypeSort = saveTarget.TypeSort
				req.Source = saveTarget.Source
				req.Version = saveTarget.Version
				req.Description = saveTarget.Description
				req.TargetType = saveTarget.TargetType
				req.SourceID = saveTarget.SourceID

				if _, _, err := lctarget.SaveTargetHistoryRecord(ctx, req); err != nil {
					log.Logger.Error("SaveToTarget err:", err)
				}
			}
		}
