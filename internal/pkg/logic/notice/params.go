...
func getCardContent(params *rao.SendCardParams) Content {
	...
	if params.ReportType == consts.PlanUI {
		for _, report := range params.UIPlanReports {
			actionButton := ActionButton{
				Tag: "button",
				Text: Text{
					Tag:     "plain_text",
				...
				Type: "primary",
				MultiURL: MultiURL{
					URL: fmt.Sprintf(conf.Conf.Base.Domain + "#/uiTestAuto/report/details/" + fmt.Sprintf("%s", report.ReportID)),
				},
			}
			actionButtons = append(actionButtons, actionButton)
		}
...
func getDingCardContent(params *rao.SendCardParams) DingActionCard {
	...
	if params.ReportType == consts.PlanUI {
		for _, report := range params.UIPlanReports {
			dingButton := DingButton{
				Title:     report.ReportName,
				ActionURL: "dingtalk://dingtalkclient/page/link?url=" + url.QueryEscape(fmt.Sprintf(conf.Conf.Base.Domain+"#/uiTestAuto/report/details/"+fmt.Sprintf("%s", report.ReportID))) + "&pc_slide=false",
			}
			dingButtons = append(dingButtons, dingButton)
		}
	}
...
func GetSendCardParamsByReq(ctx *gin.Context, req *rao.SendNoticeParams) (*rao.SendCardParams, error) {
	...
	// 自动计划报告
	if req.EventID == consts.NoticeEventUIPlan || req.EventID == consts.NoticeEventUIPlanReport {
		rx := dal.GetQuery().UIPlanReport
		reports, err := rx.WithContext(ctx).Where(rx.TeamID.Eq(req.TeamID), rx.ReportID.In(req.ReportIDs...)).Find()
		if err != nil {
			return nil, err
		}
		...
		params := &rao.SendCardParams{
			...
			RunUserName:   user.Nickname,
			Team:          team,
			ReportType:    consts.PlanUI,
			UIPlanReports: reports,
		}
