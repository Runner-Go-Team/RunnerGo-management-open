...
func SaveNoticeEvent(ctx context.Context, userID string, req *rao.SaveNoticeEventReq) error {
	...
	if req.BatchType == 0 {
		req.BatchType = consts.NoticeEventBatchTypeRepl
	}
	// 根据不同的类型进行不同的操作
	switch req.BatchType {
	case consts.NoticeEventBatchTypeAdd:
		err = saveNoticeEventBatchTypeAdd(ctx, userID, req)
	case consts.NoticeEventBatchTypeRepl:
		err = saveNoticeEventBatchTypeRepl(ctx, userID, req)
	case consts.NoticeEventBatchTypeDel:
		err = saveNoticeEventBatchTypeDel(ctx, userID, req)
	}
...
func saveNoticeEventBatchTypeAdd(ctx context.Context, userID string, req *rao.SaveNoticeEventReq) error {
	planIDs := req.PlanIDs
...
func saveNoticeEventBatchTypeRepl(ctx context.Context, userID string, req *rao.SaveNoticeEventReq) error {
	planIDs := req.PlanIDs
	groupIDs := req.GroupIDs
...
func saveNoticeEventBatchTypeDel(ctx context.Context, userID string, req *rao.SaveNoticeEventReq) error {
	planIDs := req.PlanIDs
	groupIDs := req.GroupIDs
	...
	return query.Use(dal.DB()).Transaction(func(tx *query.Query) error {
		...
		for _, groupID := range groupIDs {
			if _, err := tx.ThirdNoticeGroupEvent.WithContext(ctx).Where(
				...
				tx.ThirdNoticeGroupEvent.TeamID.Eq(req.TeamID),
				tx.ThirdNoticeGroupEvent.PlanID.In(planIDs...),
				tx.ThirdNoticeGroupEvent.GroupID.Eq(groupID),
			).Delete(); err != nil {
				return err
			}
		}
