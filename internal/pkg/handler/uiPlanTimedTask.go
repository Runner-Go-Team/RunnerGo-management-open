...
func UIPlanTimedTaskExec() {
	go updateExpiresUITask()
	...
	for {
		...
		conditions = append(conditions, tx.Status.Eq(consts.UIPlanTimedTaskInExec))
		...
		if err != nil {
			log.Logger.Info("UI自动化测试--定时任务查询数据库出错，err：", err)
			continue
		}
		...
		log.Logger.Info("UI自动化测试--定时任务查到了数据：", timedTaskData)
		// 组装运行计划参数
		for _, timedTaskInfo := range timedTaskData {
			...
			if timedTaskInfo.TaskCloseTime < nowTime {
				...
				_, err := tx.WithContext(ctx).Where(tx.PlanID.Eq(timedTaskInfo.PlanID)).
					UpdateColumn(tx.Status, consts.UIPlanTimedTaskTimeout)
				if err != nil {
					log.Logger.Info("UI自动化测试--定时任务过期状态修改失败，err：", err)
				}
				log.Logger.Info("UI自动化测试--定时任务设置为过期：", timedTaskInfo.TaskCloseTime, " 当前时间：", nowTime)
				continue
			}
			...
			// 给当前任务加分布式锁，防止重复执行
			timedTaskKey := "UIPlanTimeTaskRun:" + fmt.Sprintf("%s:%s", timedTaskInfo.TeamID, timedTaskInfo.PlanID)
			...
			// 执行定时任务计划
			err = runUITimedTask(ctx, timedTaskInfo)
			if err != nil {
				log.Logger.Info("UI自动化测试--定时任务运行失败，任务信息：", timedTaskInfo, " err：", err)
			}
		}
...
func runUITimedTask(ctx context.Context, timedTaskInfo *model.UIPlanTimedTaskConf) error {
	// 开始执行计划
	_, runErr := uiPlan.Run(ctx, timedTaskInfo.RunUserID, &rao.RunUIPlanReq{
		PlanID: timedTaskInfo.PlanID,
		TeamID: timedTaskInfo.TeamID,
	})

	log.Logger.Info("UI自动化测试定时任务--执行结果，runErr：", runErr)
	if runErr != nil {
		return runErr
	}
	return nil
...
func updateExpiresUITask() {
	for {
		ctx := context.Background()
		tx := dal.GetQuery().UIPlanTimedTaskConf
		// 组装查询条件
		conditions := make([]gen.Condition, 0)
		conditions = append(conditions, tx.Status.Eq(consts.UIPlanTimedTaskWaitEnable))
		// 从数据库当中，查出当前需要执行的定时任务
		timedTaskData, err := tx.WithContext(ctx).Where(conditions...).Select(tx.TeamID, tx.PlanID,
			tx.Frequency, tx.TaskExecTime, tx.TaskCloseTime).Find()

		if err != nil {
			log.Logger.Info("UI自动化计划测试--定时任务查询数据库出错，err：", err)
			continue
		}
		...
		log.Logger.Info("UI自动化计划测试定时任务--查到了数据：", timedTaskData)
		// 组装运行计划参数
		for _, timedTaskInfo := range timedTaskData {
			// 排除过期的定时任务
			if timedTaskInfo.TaskCloseTime < nowTime {
				...
				_, err := tx.WithContext(ctx).Where(tx.PlanID.Eq(timedTaskInfo.PlanID)).
					UpdateColumn(tx.Status, consts.UIPlanTimedTaskTimeout)
				if err != nil {
					log.Logger.Info("UI自动化计划定时任务过期状态修改失败，err：", err)
				}
				log.Logger.Info("UI自动化计划测试定时任务--设置为过期：", timedTaskInfo.TaskCloseTime, " 当前时间：", nowTime)
				continue
			}
